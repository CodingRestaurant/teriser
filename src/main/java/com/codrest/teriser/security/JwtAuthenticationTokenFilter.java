    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
        ...
        if (SecurityContextHolder.getContext().getAuthentication() == null) {
            ...
            if (authorizationToken != null) {
                try {
                    if(expiredAuthenticationTokenService.existsTokenByValue(authorizationToken)){
                    ...
                    if (nonNull(userKey) && isNotEmpty(name) && authorities.size() > 0) {
                        JwtAuthenticationToken authentication =
                                new JwtAuthenticationToken(new JwtAuthentication(userKey, name), authorizationToken, authorities);
